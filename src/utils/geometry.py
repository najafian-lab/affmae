...
import pykeops
try:
    # These helpers exist in recent PyKeOps
    print("KeOps build folder:", pykeops.get_build_folder())
    import os; os.makedirs(pykeops.get_build_folder(), exist_ok=True)
    os.environ['KEOPS_VERBOSE'] = '0'
    pykeops.set_verbose(False)
    # Optional diagnostics
    # pykeops.check_health()
except Exception as e:
    print("KeOps init diagnostics failed:", e)
from src.layers.kernels.weighted_gather import weighted_gather
from src.layers.kernels.knn import knn as triton_knn


# weight norm factor for weight initialization of GELU linear layers
GELU_GAMMA = 1.70092624336
...
# @torch.compile()
def inverse_distance_weighting(dist, power=3):
    """
    Compute the inverse-distance weighting (direct approach)
    Args:
    ...
    """
    with torch.amp.autocast(device_type='cuda', enabled=False):  # disable amp for inverse distance weighting
        dist = dist.to(torch.float32)
        ipd = 1.0 / (dist.pow(power) + 1e-10)
        weights = ipd / (ipd.sum(dim=2, keepdim=True) + 1e-12)
    return weights
...
def upsample_feature_shepard(query, database, feature, database_idx=None, k=4, power=3, custom_kernel=True, nn_idx=None, return_weight_only=False):
    ...
    dist_weight = exp(-s * (dist - d)), we can keep d fixed like 0 or 1 (more closely match to 1/x^2) and s is a learnable power
    """
    b, n_, d = database.shape
    n = query.shape[1]

    # Pick working dtype under AMP; otherwise keep input dtype
    if torch.is_autocast_enabled():
        work_dtype = torch.float16  # torch.bfloat16
        work_sec_dtype = torch.float16
    else:
        work_dtype = query.dtype
        work_sec_dtype = query.dtype

    # NOTE: this actually is FASTER when AMP (outer) is enabled
    # reason is pytorch promotes tensors to fp32 when autocast is enabled and input tensor is fp16
    # so we disable autocast here so that input tensor is fp16 and stays fp16 (I ran tests to confirm this)
    with torch.amp.autocast(device_type='cuda', enabled=False):
        database = database.to(torch.float32)  # positions are in fp32
        query = query.to(torch.float32)  # positions are in fp32
        
        # capture nearest neighbors indices
        if nn_idx is not None:
            k = nn_idx.shape[-1]
        else:
            k = min(k, n_)
            nn_idx = triton_knn(query, database, k=k, return_dist=False)
        
        # gather the nearest neighbors positions
        ...
        # apply the inverse-distance weighting (should be using target dtype)
        nn_weights = inverse_distance_weighting(nn_dist, power=power)  # b x n x k, weights of the samples
        
        # cast back to original dtype/float16/bf16 if needed (no perf gains)
        # nn_weights = nn_weights.to(work_sec_dtype)
        if return_weight_only:
            return nn_weights

        # we can move the features to the same dtype as the weights (supporting fp16/bf16)
        feature = feature.to(work_sec_dtype)
