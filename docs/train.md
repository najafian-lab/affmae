...
Single-GPU NVIDIA training defaults to FP16 and CUDA graph replay
(`train.cuda_graph: true`). Replay includes forward, native loss, and backward;
optimizer updates, gradient clipping, and GradScaler updates remain outside the
graph. Gradients accumulate across microbatches, and the current loss scale is
passed on the GPU. Warmup preserves RNG and model buffers. A changed batch shape
runs eagerly; graph capture OOM falls back to eager execution. CPU, AMD, and DDP
use eager training. Set `train.cuda_graph: false` to disable capture explicitly.

KNN defaults to `AFFMAE_KNN_BACKEND=auto`: Triton for supported 2-D positions
with at most eight neighbors, then KeOps or PyTorch for other workloads. Triton
works on CUDA and ROCm. `AFFMAE_KNN_BACKEND=keops` restores the old selection
order; disable CUDA graphs with this override. Equidistant-neighbor ordering can
differ between backends. The exact grouped attention reducer is the default for
contiguous eight-token clusters; FP16-rounded tensor-core reductions are not enabled.

Models through 1024px with 8px patches use compact positional tables matching
the released checkpoints. Larger supported models retain the full tables through
2048px; offsets are not truncated to obtain the smaller models' speedups.
