...
NVIDIA segmentation inference uses FP16 and CUDA graph replay by default in the
CLI, demo, and Python API. The first call warms up and captures the model; later
calls reuse the graph. CPU/ROCm and unsupported captures run eagerly. Use
`cuda_graph=False` to disable replay or `amp=False` for FP32 in
`AFFMAE.from_checkpoint(...)`. See [inference settings](docs/performance.md#public-inference).
...
Benchmark results generated by `scripts/benchmark.py` on an RTX 5090 (bs 4):

| Task | Resolution | ViT ms | AFF ms | Speedup | Memory reduction |
|---|---:|---:|---:|---:|---:|
| pretrain | 512 | 53.51 | 28.26 | 1.90× | 1.88× |
| pretrain | 1024 | 411.59 | 80.00 | 5.15× | 2.08× |
| finetune | 512 | 109.55 | 42.69 | 2.57× | 1.79× |
| finetune | 1024 | 942.60 | 153.47 | 6.14× | 1.91× |

Real microscopy images and paired labels; FP16 with CUDA graphs for both models
and FlashAttention for ViT. Times cover forward + loss + backward, excluding data
loading and optimizer updates. Memory reduction is ViT/AFF peak allocated memory.
...
The script saves raw measurements and a Markdown table to `output/benchmark/`.
See [benchmark details and options](docs/performance.md#reproduce-locally).
...
The default encoder KNN uses Triton on supported NVIDIA and AMD workloads. KeOps
and PyTorch provide fallbacks for other shapes and devices. Single-GPU NVIDIA
training enables FP16 and CUDA graph replay by default; see [training](docs/train.md)
for controls and supported configurations, and [performance](docs/performance.md)
for the release benchmark and its scope. Additionally,
