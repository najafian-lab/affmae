...
from affmae.layers.pos_embed import (assert_resolution_fits, pre_table,
                                     rel_pos_width, table_width)
from affmae.ops.clustering import anchor_layout, space_filling_cluster
from affmae.ops.dispatch import INDEX_DTYPE
from affmae.ops.knn_keops import knn_keops
from affmae.ops.weighted_features import weighted_features
from affmae.ops.nbhd_attn_triton import FlashNeighborhoodAttentionFunction, KVEdgeIndexCache
...
class ClusterTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, cluster_attention_backend="auto"):
        super().__init__()
        self.dim = dim
        ...
        self.attn = ClusterAttention(
            ...
            proj_drop=drop,
            backend=cluster_attention_backend
        )
...
class ClusterMerging(nn.Module):
    ...
    def __init__(self, dim, out_dim, norm_layer=nn.LayerNorm, alpha=4.0, ds_rate=0.25, reserve_on=True):
        ...
        # Relative-position table as a buffer, not a module global. It used to
        # be a module-level tensor rebound on first forward behind an `is_cuda`
        # guard -- so a second model on another device saw is_cuda=True, skipped
        # the move, and fed a cuda:0 tensor into a cuda:1 Linear. Being a global
        # rather than a buffer also meant model.to(device) could never fix it.
        self.register_buffer("pre_table", pre_table.clone(), persistent=False)
    ...
    def forward(self, pos, feat, member_idx, cluster_mask, learned_prob, stride, pe_idx, reserve_num):
        ...
        if member_idx is None:
            ...
            weights_table = self.weight_net(self.pre_table)

            weight_shape = pe_idx.shape
            inner_ch = weights_table.shape[-1]
            # Row lookup with a 1-D index; same values as gather-with-expand.
            weights = torch.index_select(weights_table, 0, pe_idx.reshape(-1)).reshape(*(weight_shape), inner_ch)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, out_dim, cluster_size, nbhd_size,
                 ...
                 layer_scale=0.0, downsample=None, cluster_attention_backend="auto"):

        super().__init__()
        self.dim = dim
        self.nbhd_size = nbhd_size
        self.rel_pos_width = rel_pos_width
        ...
        # Canvas-only precompute, keyed on shape. Per-instance: ops/ holds no
        # mutable global state.
        self._anchor_layouts = {}

        # cache the clustering result for the first feature map since it is on grid

    def _anchor_layout_for(self, h, w, k, device, dtype):
        """Cached canvas precompute for :func:`space_filling_cluster`.

        Args:
            h, w: canvas extent in token units.
            k: cluster count.
            device, dtype: of the positions; keyed so neither can go stale.
        Returns:
            What :func:`affmae.ops.clustering.anchor_layout` returns.
        """
        key = (int(h), int(w), int(k), str(device), dtype)
        hit = self._anchor_layouts.get(key)
        if hit is None:
            hit = anchor_layout(int(h), int(w), int(k), '', device)
            if len(self._anchor_layouts) >= 8:
                self._anchor_layouts.pop(next(iter(self._anchor_layouts)))
            self._anchor_layouts[key] = hit
        return hit

    @contextmanager
    def _stage_kv_cache_scope(self, enable_cache: bool):
        if not enable_cache:
            yield
            return
        prev_cache = FlashNeighborhoodAttentionFunction.kv_edge_cache
        stage_cache = KVEdgeIndexCache(
            max_entries=max(8, len(self.blocks) + 2), cluster_size=self.cluster_size,
        )
        FlashNeighborhoodAttentionFunction.set_kv_edge_cache(stage_cache)
        try:
            yield
        finally:
            FlashNeighborhoodAttentionFunction.set_kv_edge_cache(prev_cache)


    def forward(self, pos, feat, h, w, stride):
        """
        Args:
            pos - b x n x 2, token positions
            feat - b x n x c, token features
        ...
        # Local to the call. Assigning back to self.nbhd_size (as this used to)
        # made a short sequence permanently shrink the layer: the constructor
        # value was overwritten with an n-dependent rounding and never restored.
        ...
        if k == n:
            ...
            # NB: there used to be an `on_grid and self.training` fast path
            # that cached the clustering on the module and sliced it to [:b].
            # No caller ever enabled it, but had it run, the first forward with
            # a *larger* batch would have silently reused a stale, too-small
            # clustering. The now-unused `on_grid` flag went with it.
            pos, cluster_mean_pos, member_idx, cluster_mask, reorder = space_filling_cluster(
                pos, self.cluster_size, h, w, no_reorder=False,
                anchor_layout_cached=self._anchor_layout_for(h, w, k, pos.device, pos.dtype))
            # Same permutation as the two-tensor advanced index it replaces,
            # without building a host-side row index (a blocking H2D per stage).
            feat = feat.gather(dim=1, index=reorder.expand(-1, -1, c))

        assert member_idx.shape[1] == k and member_idx.shape[2] == self.cluster_size, "member_idx shape incorrect!"

        # The cast is load-bearing: knn_keops returns whichever integer dtype its
        # backend produced (keops, the Triton KNN, or cdist+topk all differ), and
        # Tensor.gather demanded an int64 index until torch relaxed it. Without
        # it, every encoder forward raises "gather(): Expected dtype int64 for
        # index" on torch 2.4 -- on ROCm and on CPU alike, since the keops path
        # is unavailable on both. INDEX_DTYPE is int32 where this build of ATen
        # accepts a narrow gather index, int64 otherwise.
        nearest_cluster = knn_keops(pos, cluster_mean_pos, nnc).to(INDEX_DTYPE)  # b x n x nnc

        m = self.cluster_size
        # Narrow the sources, not the result: [b, k, m] is nnc times smaller
        # than [b, n, nbhd]. Also gives every block in the stage one shared
        # int32 member_idx instead of a fresh copy each.
        member_idx = member_idx.to(INDEX_DTYPE)
        cluster_sel = nearest_cluster.view(b, -1, 1).expand(-1, -1, m)
        member_idx = member_idx.gather(index=cluster_sel, dim=1).reshape(b, n, nbhd_size)  # b x n x nnc*m

        if cluster_mask is not None:
            cluster_mask = cluster_mask.to(INDEX_DTYPE).gather(index=cluster_sel, dim=1).reshape(b, n, nbhd_size)

        if global_attn:
            rel_pos = (pos[:, None, :, :]+self.rel_pos_width) - pos[:, :, None, :]  # b x n x n x d
        else:
            pos_ = pos.gather(index=member_idx.view(b, -1, 1).expand(-1, -1, d), dim=1).reshape(b, n, nbhd_size, d)
            rel_pos = pos_ - (pos.unsqueeze(2)-self.rel_pos_width)  # b x n x nbhd_size x d

        rel_pos = rel_pos.clamp(0, self.table_width-1)
        # Bounded by table_width**2 - 1, so int64 only widens the index every
        # attention module in the stage then reads per element.
        pe_idx = (rel_pos[..., 1] * self.table_width + rel_pos[..., 0]).to(INDEX_DTYPE)

        selection_indices = None

        # The attention blocks want a bool mask and every block in this stage
        # sees the same one, so convert once here instead of re-deriving the
        # identical [b, n, nbhd] bool from the int64 mask inside each block.
        # ``cluster_mask`` itself stays as it was: the ClusterMerging downsample
        # below multiplies with it and relies on its dtype for promotion.
        if cluster_mask is None or cluster_mask.dtype == torch.bool:
            cluster_mask_bool = cluster_mask
        else:
            cluster_mask_bool = cluster_mask > 0

        with self._stage_kv_cache_scope(enable_cache=(not global_attn)):
        ...
        if self.downsample is not None:
            if isinstance(self.downsample, L2NormMerging) and selection_indices is not None:
                ...
                pos_down, feat_down = self.downsample(
                    pos=pos, feat=feat,
...
class AFFEncoder(nn.Module):
    def __init__(self, img_size=384, patch_size=8, in_chans=1,
                 ...
                 merging_method="l2norm", cluster_attention_backend="auto"):
        super().__init__()

        # Fail here rather than at inference: offsets past the table are
        # clamped, so an oversized input degrades silently instead of erroring.
        assert_resolution_fits(img_size, patch_size)
        ...
        # Match released checkpoint tables without shrinking larger models' coverage.
        if img_size // patch_size <= 128:
            radius = 127
            width = 2 * radius + 1
            start = rel_pos_width - radius
            compact = pre_table.view(table_width, table_width, 5)[
                start:start + width, start:start + width,
            ].reshape(-1, 5).contiguous()
            for layer in self.layers:
                layer.rel_pos_width = radius
                layer.table_width = width
                for module in layer.modules():
                    if hasattr(module, "pre_table_fp32"):
                        module.pre_table_fp32 = compact.clone()
                        module.pre_table_fp16 = compact.half()
                    elif isinstance(module, ClusterMerging):
                        module.pre_table = compact.clone()
    ...
    def forward(self, feat, pos, h, w):
        ...
        for i_layer in range(self.num_layers):
            ...
            pos_out, x_out, pos, feat = layer(pos, feat, h=h, w=w, stride=(2**(i_layer+1)))
