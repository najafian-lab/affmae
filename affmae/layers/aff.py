...
from affmae.ops.clustering import space_filling_cluster
...
class BasicLayer(nn.Module):
    ...
    def _stage_kv_cache_scope(self, enable_cache: bool):
        ...
        stage_cache = KVEdgeIndexCache(max_entries=max(8, len(self.blocks) + 2))
    ...
    def forward(self, pos, feat, h, w, stride):
        ...
        # .long() is load-bearing: knn_keops returns whichever integer dtype its
        ...
        # the cast, every encoder forward raises "gather(): Expected dtype int64
        # for index" on torch 2.4 -- on ROCm and on CPU alike, since the keops
        # path is unavailable on both.
        nearest_cluster = knn_keops(pos, cluster_mean_pos, nnc).long()  # b x n x nnc
