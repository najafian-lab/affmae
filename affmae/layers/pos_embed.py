...
#: Largest input the relative-position table can represent, and the smallest
#: patch size it must support. Offsets beyond the table are *clamped* in
#: aff.py, not rejected -- so an undersized table does not fail, it silently
#: collapses every long-range token pair onto the same positional bias. At the
#: previous 1024 the encoder therefore ran at 1536 and 2048 while quietly
#: losing positional discrimination past 127 patches.
#:
#: Cost is (2*rel_pos_width+1)^2 * 5 values, held once per attention module:
#:   1024 -> table_width 255, 46.5 MB across the 25 modules
#:   1536 -> table_width 383, 104.9 MB
#:   2048 -> table_width 511, 186.8 MB
MAX_INPUT_RESOLUTION = 2048
MIN_PATCH_SIZE = 8

rel_pos_width = MAX_INPUT_RESOLUTION // MIN_PATCH_SIZE - 1
table_width = 2 * rel_pos_width + 1


def assert_resolution_fits(img_size: int, patch_size: int) -> None:
    """Raise if this input needs relative offsets the table cannot hold.

    Called at model construction so the failure names the constant to change,
    rather than surfacing as quietly degraded accuracy at inference time.

    Args:
        img_size: input side length in pixels.
        patch_size: encoder patch size.
    Raises:
        ValueError: if the largest relative offset exceeds ``rel_pos_width``.
    """
    needed = img_size // patch_size - 1
    if needed > rel_pos_width:
        raise ValueError(
            f"img_size={img_size} with patch_size={patch_size} needs relative "
            f"offsets up to {needed}, but the position table only holds "
            f"{rel_pos_width}. Offsets past that are clamped, which silently "
            f"removes long-range positional information. Raise "
            f"MAX_INPUT_RESOLUTION in affmae/layers/pos_embed.py "
            f"(currently {MAX_INPUT_RESOLUTION}).")
