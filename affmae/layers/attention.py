...
class ClusterAttention(nn.Module):
    ...
    def _make_pos_bias(self, pe_idx: torch.Tensor, dtype: torch.dtype) -> torch.Tensor:
        ...
        # Gather along dim=0 with a 1-D index; index_select takes int32, so no
        # widening copy of B*N*M indices per block.
        flat_idx = pe_idx.reshape(-1)                                      # [BNM]
    ...
    def forward(self, feat, member_idx, cluster_mask, pe_idx, global_attn: bool):
        ...
        # One permute-copy yielding k and v both contiguous. Channel order is
        # [head][k|v][head_dim], as _heads-then-split read it, so values are
        # unchanged; the old form left strided views each backend re-copied.
        kv2 = kv.view(B, N, H, 2, C_h).permute(3, 0, 2, 1, 4).contiguous()
        k, v = kv2[0], kv2[1]                             # each [B,H,N,C_h]
