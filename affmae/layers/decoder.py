...
from .attention import DeformableSelfAttention, DeformableCrossAttention
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, pos):
        ...
        if self.normalize and n > 0:
            # n == 0 when nothing is masked. max() over an empty reduction
            # raises, and there is nothing to normalize, so skip it.
            eps = 1e-6
...
class MSDecoderBlock(nn.Module):
    ...
    def __init__(self, d_model=256, n_heads=8, n_points=4, d_ffn=1024,
                 ...
                 shepard_power_learnable=True, deform_backend="auto",
                 *, grid_h, grid_w):
        """
        Args:
            grid_h: int, patch-grid height (``img_size // patch_size``).
                Keyword-only and required -- see _DeformablePointAttention for
    ...
    def forward(self, query_tokens, query_pos, src, pos, pos_embed,
                cache_key=None, query_cache_key=None):
        """
        ...
        Args:
            ...
            cache_key: hashable identity of this stage's *source* positions
                within the active cache scope. Every block in a stage sees the
                same source positions, so a key lets them share the
                cross-attention KNN table.
            query_cache_key: hashable identity of the *query* positions. Defaults
                to ``cache_key``. The decoder passes a stage-independent key here
                because ``query_pos`` is the same tensor at every stage, so the
                self-attention table can be built once for the whole decoder
                instead of once per stage. The role is appended to both so the
                self- and cross-attention tables cannot collide.

        Returns:
            output: Processed query tokens [b, n_q, c]
        """
        # self attention on query tokens
        if query_cache_key is None:
            query_cache_key = cache_key
        self_attn_out = self.self_attn(
            query_tokens, query_pos,
            cache_key=None if query_cache_key is None
            else (query_cache_key, "self"))
        ...
        cross_attn_out = self.cross_attn(
            query_tokens, query_pos, src_with_pos, pos,
            cache_key=None if cache_key is None else (cache_key, "cross"))
...
class CrossAttentionPixelDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.deform_backend = resolve_deform_backend(deform_backend)
        self.use_knn_cache = use_knn_cache
        self.knn_grid_h = knn_grid_h
        self.knn_grid_w = knn_grid_w
    ...
    def from_config(cls, cfg, input_shape: Dict[str, str]):
        ...
        ret["deform_backend"] = getattr(cfg.MODEL.AFF, "DECODER_DEFORM_BACKEND", "csr_knn_cached")
        ret["use_knn_cache"] = getattr(cfg.MODEL.AFF, "DECODER_KNN_CACHE", True)
        knn_grid_size = cfg.INPUT.IMAGE_SIZE // cfg.MODEL.AFF.PATCH_SIZE
        ret["knn_grid_h"] = getattr(cfg.MODEL.AFF, "DECODER_KNN_GRID_H", knn_grid_size)
        ret["knn_grid_w"] = getattr(cfg.MODEL.AFF, "DECODER_KNN_GRID_W", knn_grid_size)
        return ret
    ...
    def forward_features(self, features, masked_tokens, masked_pos):
        ...
        # Stage-independent on purpose: query_pos is assigned once and never
        # reassigned below, so every stage asks for the same table. Keying it
        # per stage rebuilt that table once per stage.
        query_cache_key = ("query_pos", tuple(query_pos.shape))
        ...
        # One scope for the whole decoder so the table survives across stages.
        # Owned by this call and cleared on exit.
        decoder_cache_ctx = cache_scope(
            name="decoder",
            max_entries=2 * self.transformer_num_feature_levels + 4,
        ) if self.use_knn_cache else nullcontext()

        # from res5 to res2
        with decoder_cache_ctx:
            for stage_idx in range(self.transformer_num_feature_levels):
                with record_function(f"aff.decoder.stage_{stage_idx}"):
                    src = srcs[stage_idx]
                    pos = poss[stage_idx]
                    pos_embed = pos_embeds[stage_idx]
                    stage_blocks = self.decoder_blocks[stage_idx]
                    stage_name = self.transformer_in_features[::-1][stage_idx]
                    # No data_ptr(): freed addresses are reused, so it could
                    # serve a stale entry. Name, index and shapes are stable.
                    cache_scope_key = (
                        stage_name,
                        stage_idx,
                        tuple(query_pos.shape),
                        tuple(pos.shape),
                    )

                    for block_idx, block in enumerate(stage_blocks):
                        query_tokens = block(
                            cache_key=cache_scope_key,
