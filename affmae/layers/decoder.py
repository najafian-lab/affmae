...
class MSDecoderBlock(nn.Module):
    ...
    def forward(self, query_tokens, query_pos, src, pos, pos_embed,
        ...
        Args:
            ...
            cache_key: hashable identity of this stage's positions within the
                active cache scope. Every block in a stage sees the same query
                and source positions, so a key lets them share both KNN tables.
                The role is appended so the self- and cross-attention tables
                cannot collide when both use the same grid.
        ...
        self_attn_out = self.self_attn(
            ...
            cache_key=None if cache_key is None else (cache_key, "self"))
    ...
    def forward_features(self, features, masked_tokens, masked_pos):
        ...
        for stage_idx in range(self.transformer_num_feature_levels):
            with record_function(f"aff.decoder.stage_{stage_idx}"):
                ...
                # Identity of the query positions for this stage. Note there is
                # no data_ptr() here: storage addresses are reused after a free,
                # so a stale entry could be served for unrelated positions. The
                # stage name, index and shapes are sufficient and stable.
                cache_scope_key = (
                    stage_name,
                    stage_idx,
                ...
                # The cache is an object owned by this call, not a class
                # attribute, so two models in one process cannot collide. It is
                # cleared when the scope exits.
                cache_ctx = cache_scope(name=f"decoder_stage_{stage_idx}") \
