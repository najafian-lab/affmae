...
class AFFMaskedAutoEncoder(nn.Module):
    ...
    def forward_encoder(self, x):
        ...
        visible_tokens = torch.gather(feat, dim=1, index=ids_keep.unsqueeze(-1).expand(-1, -1, feat.shape[-1]))
        visible_pos = torch.gather(pos, dim=1, index=ids_keep.unsqueeze(-1).expand(-1, -1, 2))
    ...
    def mask_and_embed(self, x):
        ...
        # .expand, not .repeat: gather takes its expanded-index fast path
        # instead of materialising a [B, L_keep, C] int64 tensor. Same values.
        visible_tokens = torch.gather(
            feat, dim=1,
            index=ids_keep.unsqueeze(-1).expand(-1, -1, feat.shape[-1]))
        visible_pos = torch.gather(
            pos, dim=1, index=ids_keep.unsqueeze(-1).expand(-1, -1, 2))
    ...
    def _forward_internal(self, x):
        ...
        masked_pos = torch.gather(pos, dim=1, index=ids_masked.unsqueeze(-1).expand(-1, -1, 2))
        ...
        masked_patches_gt = torch.gather(img_patches, dim=1, index=ids_masked.unsqueeze(-1).expand(-1, -1, img_patches.shape[2]))
