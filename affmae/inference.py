...
class AFFMAE:
    ...
    * either one can show the adaptive token layout -- :meth:`token_layout`
    ...
    def __init__(self, model, img_size: int, num_classes: int, device,
                 ...
                 cuda_graph: bool = True, amp: bool = True):
        from affmae.ops.inference_graph import InferenceGraph
        ...
        self._inference_graph = InferenceGraph(
            model, enabled=cuda_graph and self.mode is Mode.INFERENCE and self.num_classes > 0)

    def clear_inference_cache(self):
        """Clear captures after replacing model parameters or configuration."""
        self._inference_graph.clear()

    def _inference_forward(self, images, stages=False):
        def step(tensor):
            with _capture_encoder_stages(self.model) as captured:
                output = self.model(tensor)
            return output, captured

        use_amp = (self.amp and self.device.type == "cuda" and torch.version.hip is None
                   and not torch.is_autocast_enabled("cuda"))
        context = torch.autocast("cuda", dtype=torch.float16) if use_amp else nullcontext()
        # Captured buffers must survive changes to the caller's inference mode.
        with torch.inference_mode(False), torch.no_grad(), context:
            return self._inference_graph.run(step, images.to(self.device), stages=stages)
    ...
    def from_checkpoint(cls, checkpoint: str, config=None,
                        ...
                        cuda_graph: bool = True, amp: bool = True) -> "AFFMAE":
        ...
        Args:
            ...
            cuda_graph: cache CUDA graphs for segmentation on NVIDIA (default True).
                Unsupported captures and shapes/precision modes beyond two run eagerly.
            amp: use FP16 autocast for NVIDIA inference (default True). Set False
                for FP32; an outer autocast context takes precedence.
        ...
        return cls(model.to(resolved).eval(), cfg.img_size,
                   ...
                   mode=resolved_mode, cuda_graph=cuda_graph, amp=amp)
    ...
    def from_model(cls, model, img_size: int, num_classes: int,
        ...
        Args:
            ...
            cuda_graph: enable bounded NVIDIA segmentation graph replay.
            amp: enable FP16 NVIDIA inference; an outer autocast takes precedence.
        Returns:
        ...
        return cls(model, img_size, num_classes, device,
                   patch_size=patch_size, mode=mode, cuda_graph=cuda_graph, amp=amp)
    ...
    def segment(self, source: ImageSource) -> SegmentationResult:
        ...
        if self.mode is Mode.INFERENCE:
            outputs, captured = self._inference_forward(tensor, stages=True)
        else:
            with _capture_encoder_stages(self.model) as captured:
