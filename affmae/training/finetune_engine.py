...
def train_epoch(model, dataloader, optimizer, loss_fn, epoch, loss_scaler, cfg,
                global_step, aux_names, throttle_s=0.0, graph_runner=None):
    """Train one epoch.
    ...
    if graph_runner is None:
        graph_runner = TrainingGraph(model, enabled=getattr(cfg, "cuda_graph", True))

    def microbatch(images, targets, scale):
        with torch.amp.autocast("cuda", dtype=amp_dtype):
            pred_logits = model(images)
            finite = torch.stack([torch.isfinite(logits).all() for logits in pred_logits])
            per_head, loss = _combine_losses(pred_logits, targets, loss_fn)
            loss = loss / num_accum
        (loss * scale).backward()
        return loss.detach(), tuple(value.detach() for value in per_head), finite

    for batch_idx, (images, targets, _) in enumerate(dataloader):
        images = images.to(cfg.device, non_blocking=True)
        targets = targets.to(cfg.device, non_blocking=True).long()
        loss, per_head, finite = graph_runner.run(microbatch, (images, targets), loss_scaler)
        if not finite.all():
            optimizer.zero_grad(set_to_none=True)
            raise FloatingPointError(
                f"Non-finite logits at epoch {epoch} batch {batch_idx}: "
                f"finite_per_head={finite.tolist()} "
...
def run_finetune(base_cfg, seed=None, train_loader_fn=None, wandb_run=None,
    ...
    for epoch in range(cfg.epochs):
        train_loss, current_lr, global_step = train_epoch(
            ...
            global_step, spec.aux_names, throttle_s=throttle_s, graph_runner=graph_runner)
