...
from affmae.training.graph import TrainingGraph
...
def train_epoch(model: torch.nn.Module, dataloader: DataLoader, optimizer: torch.optim.Optimizer,
                    ...
                    total_max_steps: int, aux_names=(), params=None, graph_runner=None) -> tuple[float, int]:
    model.train()
    ...
    if graph_runner is None:
        graph_runner = TrainingGraph(model, enabled=getattr(config, "cuda_graph", True) and AMP)

    def microbatch(samples, scale):
        with (autocast_context(config.device) if AMP else nullcontext()):
            loss, aux = model(samples)
            loss = loss / num_accum
        (loss * scale).backward()
        return loss.detach(), tuple(value.detach() for value in aux)

    for batch_idx, (samples, _) in enumerate(dataloader):
        samples = samples.to(config.device, non_blocking=True)
        loss, aux_losses_list = graph_runner.run(microbatch, (samples,), loss_scaler if AMP else None)
...
def run_pretrain(config, config_path):
    ...
    loss_scaler = torch.amp.GradScaler(enabled=AMP)
    graph_runner = TrainingGraph(model, enabled=getattr(config, "cuda_graph", True) and AMP)
