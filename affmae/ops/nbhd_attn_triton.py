...
class KVEdgeIndexCache:
    ...
    Args:
        ...
        cluster_size: Set to 8 only when member lists are whole contiguous
            eight-token AFF clusters in reordered token space. Zero retains
            the generic arbitrary-neighborhood representation.
    """
    def __init__(self, max_entries: int = 16, cluster_size: int = 0):
    ...
    def get_or_build(self, member_index: torch.Tensor):
        ...
        if self.cluster_size == 8:
            from .nbhd_attn_clustered import build
            built = build(member_index)
        else:
...
def _ensure_kv_edge_index_fields(edge_index: dict):
    if edge_index.get("clustered", False):
        return edge_index
...
def _build_kv_edge_index(member_index: torch.Tensor):
    bsz, n_ctx, n_nb = member_index.shape
    ids = member_index.reshape(bsz, n_ctx * n_nb).long()
    sorted_ids, order = torch.sort(ids, dim=1, stable=True)
    # Fixed-size batched counts avoid bincount's host synchronization during capture.
    counts = torch.zeros((bsz, n_ctx), device=member_index.device, dtype=torch.int32)
    counts.scatter_add_(1, ids, torch.ones_like(ids, dtype=torch.int32))
    row_ptr = torch.cat((
        torch.zeros((bsz, 1), device=member_index.device, dtype=torch.int32),
        counts.cumsum(1, dtype=torch.int32),
    ), dim=1)
    return {
        "row_ptr": row_ptr,
        "edge_flat": order.int(),
        "edge_q": (order // n_nb).int(),
        "edge_m": (order % n_nb).int(),
        "kv_sorted": sorted_ids.int(),
        "kv_order": torch.argsort(counts, dim=1, descending=True).int(),
        "n_ctx": n_ctx,
        "n_nb": n_nb,
    }
...
def _validate_kv_edge_index(edge_index: dict, member_index: torch.Tensor):
    if edge_index.get("clustered", False):
        b, n, m = member_index.shape
        if edge_index["n_ctx"] != n or edge_index["n_nb"] != m or m % 8:
            raise ValueError("cluster edge metadata mismatch")
        if edge_index["row_ptr"].shape != (b, (n + 7) // 8 + 1):
            raise ValueError("cluster row pointer shape mismatch")
        if edge_index["cluster_edges"].shape != (b, n * (m // 8)):
            raise ValueError("cluster edge shape mismatch")
        return
    bsz, n_ctx, n_nb = member_index.shape
    e = n_ctx * n_nb
...
def _kv_owner_reduce_from_edges_triton(
    ...
    if edge_index.get("clustered", False):
        from .nbhd_attn_clustered import reduce
        return reduce(q_, do_, ds_flat, p_flat, edge_index, sm_scale, h)
    bh, n_ctx, head_dim = q_.shape
    ...
    if backend in {"atomic_split", "kv_owner"}:
        ...
        if backend == "atomic_split":
            ...
            if use_python_reduce:
                ...
                if kv_edge_index.get("clustered", False):
                    kv_edge_index = _build_kv_edge_index(mi.view(bsz, n, m))
