...
def nbhd_attn(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
    ...
    # aff.py hands this over already narrowed and contiguous, so both calls are
    # no-ops; otherwise .contiguous() copies [B, N, M] once per block.
    if member_idx.dtype == torch.int32 and member_idx.is_contiguous():
        member_i32 = member_idx
    else:
