"""Balanced token clustering along a space-filling curve.

Self-contained: ``math`` and ``torch``, no global state, no imports from the rest
of AFF-MAE. Copy this file into another project and it works.

This is the token-grouping half of AFF's adaptive downsampling. It orders tokens
along a space-filling curve (scanline by default, optionally Peano or Hilbert)
and cuts that order into equal-size clusters, so every cluster holds ``m`` tokens
and only the last carries padding — which is what the returned mask marks.

    from affmae.ops import SpaceFillingCluster

    cluster = SpaceFillingCluster(cluster_size=8, sf_type="hilbert")
    pos, centers, member_idx, mask, order = cluster(positions, h=64, w=64)

The functional form (:func:`space_filling_cluster`) is the same algorithm without
the module wrapper.
"""
...
def anchor_layout(h, w, k, sf_type="", device=None):
    """Anchor grid, curve order and neighbour means for one canvas.

    Nothing here depends on the token positions -- only on the canvas and the
    cluster count -- so a caller that runs the same shapes every step can build
    this once and hand it back through ``space_filling_cluster(...,
    anchor_layout=...)``. Doing so removes, per call, two aranges, a meshgrid,
    a sort, a scatter, an advanced index, four slice-assign copies, and two
    *pageable* host-to-device copies (the old ``torch.arange(...).to(device)``
    and ``torch.Tensor([...]).to(device)``, both of which synchronise).

    No cache lives here: ``affmae/ops`` holds no module-level mutable state by
    design, so ownership of the memo belongs to the caller (see
    ``BasicLayer._anchor_layout_for`` in ``affmae/layers/aff.py``).

    Args:
        h: int, canvas height in token units.
        w: int, canvas width in token units.
        k: int, number of clusters.
        sf_type: str, ``"peano"``, ``"hilbert"`` or anything else for scanlines.
        device: torch.device or None for CPU.
    Returns:
        ``(order_grid_idx, patch_len_hw, prev_means, next_means, num_patch_w)``.
    Raises:
        ValueError: if the canvas yields fewer than three anchors.
    """
    ...
    if sf_type == 'peano':
        ...
        patch_len_w = w / num_patch_w
    hs = torch.arange(0, num_patch_h, device=device)
    ws = torch.arange(0, num_patch_w, device=device)
    ...
    if sf_type == 'peano':
        ...
        order_idx_src = torch.arange(len(order_idx), device=device)
    ...
    patch_len_hw = torch.tensor([patch_len_w, patch_len_h], device=device,
                                dtype=torch.float32)

    init_pos_means = ordered_grid * patch_len_hw + patch_len_hw/2 - 0.5
    nump = ordered_grid.shape[0]

    # The refinement below reads prev_means[2] and next_means[-3], so it needs
    # at least three anchors.
    if nump < 3:
        raise ValueError(
            f"space_filling_cluster: only {nump} anchor patch(es) for "
            f"k={k} clusters on a {h}x{w} canvas, but the anchor refinement "
            f"needs at least 3. Use a larger input, a smaller cluster size, or "
            f"pass use_anchor=False.")
    ...
    return order_grid_idx, patch_len_hw, prev_means, next_means, num_patch_w


def space_filling_cluster(pos, m, h, w, no_reorder=False, sf_type='', use_anchor=True,
                          anchor_layout_cached=None):
    """
    ...
    Args:
        ...
        anchor_layout_cached - the tuple returned by :func:`anchor_layout` for this
                            (h, w, k, sf_type, device), or None to build it here.
                            It depends only on the canvas, so a caller running
                            fixed shapes should build it once and reuse it.
    Returns:
    ...
    with torch.no_grad():
        ...
        if use_anchor:
            # Depends only on the canvas and cluster count, not on `pos`, so a
            # caller with fixed shapes can pass it in.
            (order_grid_idx, patch_len_hw, prev_means, next_means,
             num_patch_w) = (
                anchor_layout_cached if anchor_layout_cached is not None
                else anchor_layout(int(h), int(w), int(k), sf_type, pos.device))
...
class SpaceFillingCluster(torch.nn.Module):
    """Balanced space-filling-curve clustering as an ``nn.Module``.

    Holds no parameters and no buffers — it exists so the operator can be
    dropped into a ``nn.Sequential`` or configured once and reused, rather than
    threading five keyword arguments through every call site.

    Args:
        cluster_size: int, target tokens per cluster (``m``).
        sf_type: str, ``"peano"``, ``"hilbert"``, or anything else for
            alternating-direction scanlines (the default).
        use_anchor: bool, order anchor points rather than the tokens themselves.
        no_reorder: bool, keep the original token order instead of grouping
            cluster members contiguously.
    """

    def __init__(self, cluster_size: int, sf_type: str = "",
                 use_anchor: bool = True, no_reorder: bool = False):
        super().__init__()
        if cluster_size < 1:
    ...
    def forward(self, pos: torch.Tensor, h: int, w: int):
        """Cluster ``pos``.

        Args:
            pos: [b, n, 2] token positions.
            h: int, canvas height in token units.
            w: int, canvas width in token units.
        Returns:
            With ``no_reorder=False``:
            ``(pos, cluster_mean_pos, member_idx, cluster_mask, pos_ranking)``.
            With ``no_reorder=True``: ``(cluster_mean_pos, member_idx,
            cluster_mask)``. ``cluster_mask`` is None when ``n`` divides evenly
            by ``cluster_size``.
        """
        return space_filling_cluster(
            pos, self.cluster_size, h, w,
            no_reorder=self.no_reorder, sf_type=self.sf_type,
            use_anchor=self.use_anchor,
        )

    def extra_repr(self) -> str:
        return (f"cluster_size={self.cluster_size}, sf_type={self.sf_type!r}, "
                f"use_anchor={self.use_anchor}, no_reorder={self.no_reorder}")
