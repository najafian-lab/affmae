...
def space_filling_cluster(pos, m, h, w, no_reorder=False, sf_type='', use_anchor=True):
    ...
    with torch.no_grad():
        ...
        if use_anchor:
            ...
            # The anchor refinement below reads prev_means[2] and
            # next_means[-3], so it needs at least three anchors. Fewer means
            # the token count is too small for this cluster size; say so rather
            # than raising IndexError from the middle of the algorithm.
            if nump < 3:
                raise ValueError(
                    f"space_filling_cluster: only {nump} anchor patch(es) for "
                    f"n={pos.shape[1]} tokens at cluster size m={m} on a "
                    f"{h}x{w} canvas, but the anchor refinement needs at least "
                    f"3. Use a larger input, a smaller cluster size, or pass "
                    f"use_anchor=False.")
