...
def _probe_gather_index_dtype():
    """Widest-but-cheapest index dtype ``Tensor.gather`` accepts here.

    ``gather`` required int64 indices until PyTorch relaxed it; ``index_select``
    has always taken int32 as well. Halving the index width halves the bytes the
    CUDA gather kernel reads per slice, which matters because AFF gathers one
    2-float slice per (token, neighbour). Probed once at import on a two-element
    tensor rather than branching on ``torch.__version__``, because the check is
    about the installed ATen, not the version string.
    """
    try:
        torch.zeros(2, 1).gather(0, torch.zeros(2, 1, dtype=torch.int32))
    except (RuntimeError, TypeError):
        return torch.int64
    return torch.int32


#: int32 where ``Tensor.gather`` accepts it, int64 otherwise (torch < ~2.6).
INDEX_DTYPE = _probe_gather_index_dtype()


def gather_index_dtype():
    """Return :data:`INDEX_DTYPE`. A function so tests can monkeypatch it."""
    return INDEX_DTYPE


def _probe_backend():
    """Ask the Triton driver which backend is live.
...
#: Resolved once, at import: a device does not change vendor mid-process, and
#: ``CudaDriver.get_current_target`` is in TorchDynamo's SKIP_DIRS, so probing
#: per call broke the compiled graph on every ``can_use_triton``.
_TRITON_BACKEND = _probe_backend()

#: Same for KeOps: ``find_spec`` stats the filesystem per call, is likewise
#: untraceable, and the answer cannot change within a process.
_HAS_PYKEOPS = importlib.util.find_spec("pykeops") is not None
...
def has_triton_backend() -> bool:
    ...
    return _TRITON_BACKEND is not None


def reset_backend_probe_cache():
    """Re-run the import-time probes. For tests that change the environment."""
    global _TRITON_BACKEND, _HAS_PYKEOPS
