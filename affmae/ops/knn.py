...
class CachedKNN(nn.Module):
    ...
    def forward(self, kv_pos: torch.Tensor, cache=None,
        ...
        key = ("dense_top4_knn", cache_key, self.grid_h, self.grid_w,
               str(kv_pos.device), self.backend)
        # Inside the lambda: as six kernels before the lookup, a cache hit
        # still paid for them.
        return resolved.get_or_compute(key, lambda: self._build(kv_pos))

    def _build(self, kv_pos):
        """Clamp to the lattice and run the chosen implementation."""
        return self._impl(clamp_to_grid(kv_pos, self.grid_h, self.grid_w))
