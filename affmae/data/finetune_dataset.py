...
# Defined in affmae/data/stats.py, which has no dependencies; re-exported
# here so existing imports keep working.
from affmae.data.stats import IMAGE_MEAN, IMAGE_STD  # noqa: F401
THRESHOLD = 10
...
def build_finetune_dataloader(cfg, is_train: bool):
    ...
    return DataLoader(
        ...
        # Pinned staging + a deeper prefetch queue: the per-sample cost here is
        # CLAHE, a per-class mask loop and ElasticTransform, all single-threaded,
        # so the loader starves the GPU unless several workers run ahead.
        pin_memory=True,
